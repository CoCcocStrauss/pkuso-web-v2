...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!resendKey) {
      throw new Error(
        "[EMAIL] 缺少 Resend API Key。请在环境变量中设置 RESEND_API_KEY 或 NEXT_PUBLIC_RESEND_API_KEY",
      );
    }
    ...
    console.log("[EMAIL] 收到的排练数据:", body);
    ...
    if (!title || !dateStr || !location) {
      throw new Error("[EMAIL] 缺少 title、dateStr 或 location");
    }
    ...
    if (dbError) throw new Error("[EMAIL] 数据库查询失败: " + dbError.message);
    if (!users || users.length === 0) throw new Error("[EMAIL] 没有找到可发送的邮箱");

    const emails = (users as Array<{ email: string }>).map((u) => u.email);
    console.log("[EMAIL] 准备发送给以下邮箱:", emails);
    ...
    if (sendError) throw new Error("[EMAIL] Resend 发送失败: " + sendError.message);
